...
import org.prography.lemorning.src.viewmodels.CreateSongViewModel
import java.io.IOException
import java.util.*

class CreateSongActivity
    : BaseActivity<ActivityCreateSongBinding, CreateSongViewModel>(R.layout.activity_create_song) {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        binding.mbtnRecordCreateSong.setOnClickListener {
            if (vm.isRecording == null || !vm.isRecording!!) {
        ...
        binding.mbtnSubmitCreateSong.setOnClickListener {
            if (tempFile.isNotEmpty() && vm.profileUri.value != null && binding.etTitleCreateSong.text.isNotBlank())
                vm.registerNewSong(name = binding.etTitleCreateSong.text.toString(), alarmPath = tempFile)
        }
        ...
        /* Data Observing */
        vm.canFinish.observe(this) { it.get()?.let { if (it) finish() } }
        vm.curAmpitude.observe(this) { binding.arvCreateSong.update(it) }
    }

    private fun toggleTimerForAudio(mediaRecorder: MediaRecorder) {
        if (vm.isRecording == null) {
            vm.registerTimerOnAudio(mediaRecorder)
        }
    ...
    override fun onDestroy() {
        vm.isRecording?.let { if (!it)  mediaRecorder?.release() }
        super.onDestroy()
    }
}